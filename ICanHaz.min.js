...
var h=[],b=[],z=[],k=!1,g=!1,m,n,t;c(e||f.tags);for(var p=new r(a),v,l,u,w;!p.eos();){v=p.pos;if(u=p.scanUntil(m)){w=0;for(var y=u.length;w<y;++w)if(l=u.charAt(w),D.call(E,l)?g=!0:z.push(b.length),b.push(["text",l,v,v+1]),v+=1,"\n"===l){if(k&&!g)for(;z.length;)delete b[z.pop()];else z=[];g=k=!1}}if(!p.scan(m))break;k=!0;l=p.scan(F)||"name";p.scan(G);"="===l?(u=p.scanUntil(B),p.scan(B),p.scanUntil(n)):"{"===l?(u=p.scanUntil(t),p.scan(H),p.scanUntil(n),l="&"):u=p.scanUntil(n);if(!p.scan(n))throw Error("Unclosed tag at "+
p.pos);w=[l,u,v,p.pos];b.push(w);if("#"===l||"^"===l)h.push(w);else if("/"===l){l=h.pop();if(!l)throw Error('Unopened section "'+u+'" at '+v);if(l[1]!==u)throw Error('Unclosed section "'+l[1]+'" at '+v);}else"name"===l||"{"===l||"&"===l?g=!0:"="===l&&c(u)}if(l=h.pop())throw Error('Unclosed section "'+l[1]+'" at '+p.pos);return q(x(b))}function x(a){for(var e=[],c,h,b=0,d=a.length;b<d;++b)if(c=a[b])"text"===c[0]&&h&&"text"===h[0]?(h[1]+=c[1],h[3]=c[3]):(e.push(c),h=c);return e}function q(a){for(var e=
...
function(a,b,c,d){var f=b.lookup(a[1]);if(!f||A(f)&&0===f.length)return this.renderTokens(a[4],b,c,d)};m.prototype._renderPartial=function(a,e,c){if(c&&(a=b(c)?c(a[1]):c[a[1]],null!=a))return this.renderTokens(this.parse(a),e,c,a)};m.prototype._unescapedValue=function(a,b){var c=b.lookup(a[1]);if(null!=c)return c};m.prototype._escapedValue=function(a,b){var c=b.lookup(a[1]);if(null!=c)return f.escape(c)};m.prototype._rawValue=function(a){return a[1]};f.name="mustache.js";f.version="1.0.0";f.tags=
["{{","}}"];var y=new m;f.clearCache=function(){return y.clearCache()};f.parse=function(a,b){return y.parse(a,b)};f.render=function(a,b,c){return y.render(a,b,c)};f.to_html=function(a,d,c,h){a=f.render(a,d,c);if(b(h))h(a);else return a};f.escape=function(a){return String(a).replace(/[&<>"'\/]/g,function(a){return J[a]})};f.Scanner=r;f.Context=n;f.Writer=m});(function(){function f(b){return"".trim?b.trim():b.replace(/^\s+/,"").replace(/\s+$/,"")}var b={VERSION:"0.10.2",templates:{},$:"undefined"!==
typeof window?window.jQuery||window.Zepto||null:null,addTemplate:function(d,t){if("object"===typeof d)for(var x in d)this.addTemplate(x,d[x]);else b[d]?console.error("Invalid name: "+d+"."):b.templates[d]?console.error('Template "'+d+'  " exists'):(b.templates[d]=t,b[d]=function(q,r){q=q||{};var n=Mustache.to_html(b.templates[d],q,b.templates);return b.$&&!r?b.$(f(n)):n},"undefined"!==typeof b.$&&$(document).trigger("template-loaded",[d]))},clearAll:function(){for(var d in b.templates)delete b[d];
